...
import { Shield, CheckCircle, XCircle, Clock, Lightbulb, AlertTriangle, TrendingUp } from 'lucide-react';
...
export function GovernanceDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'approvals' | 'recommendations' | 'audit'>('approvals');
  ...
  const tabs = [
    ...
    { id: 'recommendations' as const, label: 'AI Recommendations', count: pendingRecs.length },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600 mt-1">Approvals, AI recommendations, and audit trail</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <div className="text-2xl font-bold text-slate-900">{pendingRecs.length}</div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`pb-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.id ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
              ...
              {tab.count !== undefined && tab.count > 0 && (
                <span className="ml-2 text-xs bg-orange-100 text-orange-800 px-1.5 py-0.5 rounded-full">{tab.count}</span>
              )}
            </button>
          ))}
        </nav>
                ...
                <div key={approval.id} className="border border-slate-200 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <div className="text-sm text-slate-600 mt-1">Created {new Date(approval.created_at).toLocaleString()}</div>
                    </div>
                    <span className="px-2 py-1 bg-orange-100 text-orange-800 text-xs font-medium rounded">{approval.status}</span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleApprove(approval.entity_id)} className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white text-sm font-medium rounded hover:bg-green-700">
                      <CheckCircle size={16} /> Approve
                    </button>
                    <button onClick={() => handleReject(approval.entity_id)} className="flex items-center gap-1 px-3 py-1.5 bg-red-600 text-white text-sm font-medium rounded hover:bg-red-700">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-3">
            {recommendations.length > 0 ? (
                ...
                <div key={rec.id} className="border border-slate-200 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      ...
                      {rec.assets?.name && (
                        <div className="text-xs text-slate-500 mt-1">Asset: {rec.assets.name} ({rec.assets.asset_tag})</div>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      {rec.confidence_score && (
                        <span className="text-xs text-slate-500 flex items-center gap-1">
                          <TrendingUp size={12} /> {rec.confidence_score}% confidence
                        </span>
                      )}
                      <span className={`px-2 py-1 text-xs font-medium rounded ${priorityColors[rec.priority] || 'bg-slate-100 text-slate-600'}`}>
                        {rec.priority}
                      </span>
                    </div>
                  </div>
                  ...
                  {rec.status === 'pending' && (
                    <div className="flex gap-2">
                      <button onClick={() => handleAcceptRec(rec.id)} className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700">
                        <CheckCircle size={16} /> Accept
                      </button>
                      <button onClick={() => handleDismissRec(rec.id)} className="flex items-center gap-1 px-3 py-1.5 bg-slate-200 text-slate-700 text-sm font-medium rounded hover:bg-slate-300">
                        Dismiss
                      </button>
                    </div>
                  )}
                  {rec.status !== 'pending' && (
                    <span className={`text-xs px-2 py-1 rounded ${rec.status === 'accepted' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-600'}`}>
                      {rec.status}
                    </span>
                  )}
                </div>
              ...
              <div className="text-center text-slate-500 py-8">No AI recommendations at this time</div>
            )}
          </div>
        </div>
      )}
                ...
                <div key={event.id} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-slate-500 mt-1">
                      {new Date(event.event_time || event.created_at).toLocaleString()} {event.actor_id && `by ${event.actor_type}`}
                    </div>
                  </div>
                </div>
              ))
