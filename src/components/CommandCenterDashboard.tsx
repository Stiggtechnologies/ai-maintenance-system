...
interface AssetNode {
  id: string;
  name: string;
  type: 'enterprise' | 'site' | 'area' | 'system' | 'asset' | 'component' | 'sensor';
...
interface WorkOrder {
  ...
  asset: string;
  task: string;
  status: 'pending' | 'scheduled' | 'in_progress' | 'blocked' | 'completed' | 'closed';
  technician?: string;
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="text-xs text-gray-400 mb-1">{insight.asset}</div>
  ...
  return (
    <div className="h-full overflow-auto">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-gray-500">{workOrders.length} active</span>
      </div>
...
function EventFeed({ 
  ...
  return (
    <div className="h-full overflow-auto">
      ...
      <div className="space-y-2">
        {events.map((event, i) => (
          <div key={i} className="flex gap-3 text-sm py-2 border-b border-[#2A3344] last:border-0">
            <span className="text-gray-500 text-xs whitespace-nowrap">{event.time}</span>
  ...
  return (
    <div className="h-full overflow-auto">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-gray-500">{agents.filter(a => a.status === 'active').length} active</span>
      </div>
...
function FinancialImpactWidget({ 
  ...
  return (
    <div className="p-3 bg-[#1A1F2E] rounded-lg border border-[#2A3344]">
      <div className="text-xs text-gray-400 uppercase mb-2">FINANCIAL IMPACT</div>
...
export function CommandCenterDashboard() {
  ...
  const kpis = [
    { label: 'Asset Availability', value: '94.2', change: 2.1, trend: 'up' as const },
    { label: 'MTBF', value: '132', change: 8, trend: 'up' as const, unit: 'hrs' },
    { label: 'Backlog', value: '218', change: -12, trend: 'down' as const },
    { label: 'OEE', value: '87.3', change: 1.5, trend: 'up' as const, unit: '%' },
    { label: 'MTTR', value: '4.2', change: -0.5, trend: 'down' as const, unit: 'hrs' },
    { label: 'AI Actions', value: '17', change: 5, trend: 'up' as const },
  ];

  const assetHierarchy: AssetNode[] = [
    {
      id: 'ent-1', name: 'SyncAI Operations', type: 'enterprise', status: 'healthy',
      children: [
        {
          id: 'site-1', name: 'Alberta Oil Sands', type: 'site', status: 'healthy',
          children: [
            {
              id: 'area-1', name: 'Primary Extraction', type: 'area', status: 'warning',
              children: [
                {
                  id: 'sys-1', name: 'Haul Truck Fleet', type: 'system', status: 'healthy',
                  children: [
                    { id: 'asset-1', name: 'Haul Truck #12', type: 'asset', status: 'healthy' },
                    { id: 'asset-2', name: 'Haul Truck #23', type: 'asset', status: 'critical' },
                  ]
                }
              ]
            },
            {
              id: 'area-2', name: 'Processing Plant', type: 'area', status: 'healthy',
              children: [
                {
                  id: 'sys-2', name: 'Pump Station', type: 'system', status: 'warning',
                  children: [
                    { id: 'asset-3', name: 'Pump P-145', type: 'asset', status: 'warning' },
                  ]
                }
              ]
            }
  ...
  const insights: AIInsight[] = [
    {
      id: '1', type: 'prediction', severity: 'critical', asset: 'Pump P-145',
      message: 'Failure probability: 82%. Recommend bearing replacement at next shutdown.',
      impact: 420000, confidence: 0.87, timestamp: new Date()
    },
    {
      id: '2', type: 'anomaly', severity: 'high', asset: 'Haul Truck #23',
      message: 'Vibration anomaly detected in engine component. Schedule inspection.',
      impact: 85000, confidence: 0.72, timestamp: new Date()
    },
    {
      id: '3', type: 'recommendation', severity: 'medium', asset: 'Conveyor C-22',
      message: 'Belt wear at 78%. Schedule replacement during next planned outage.',
      impact: 35000, confidence: 0.91, timestamp: new Date()
    }
  ];

  const workOrders: WorkOrder[] = [
    { id: 'WO-20431', asset: 'Truck #45', task: 'Hydraulic Pump Replacement', status: 'in_progress', technician: 'Alex M.', priority: 'critical' },
    { id: 'WO-20430', asset: 'Conveyor C-22', task: 'Belt Inspection', status: 'scheduled', priority: 'high' },
    { id: 'WO-20429', asset: 'Pump P-145', task: 'Bearing Check', status: 'pending', priority: 'medium' },
  ];

  const events = [
    { time: '10:58', message: 'Maintenance task started', type: 'action' },
    { time: '10:42', message: 'Technician assigned - Alex M.', type: 'assignment' },
    { time: '10:35', message: 'AI created work order WO-20431', type: 'ai' },
    { time: '10:32', message: 'Vibration anomaly detected - Pump P-145', type: 'alert' },
  ];

  const agents: AgentStatus[] = [
    { name: 'Reliability Agent', status: 'active', tasksExecuted: 23, lastActivity: new Date() },
    { name: 'Condition Monitor', status: 'active', tasksExecuted: 156, lastActivity: new Date() },
    { name: 'Planning Agent', status: 'active', tasksExecuted: 12, lastActivity: new Date() },
    { name: 'Inventory Agent', status: 'idle', tasksExecuted: 8, lastActivity: new Date() },
    { name: 'Compliance Agent', status: 'active', tasksExecuted: 5, lastActivity: new Date() },
    { name: 'Safety Agent', status: 'active', tasksExecuted: 31, lastActivity: new Date() },
  ];
    ...
    <div className="min-h-screen bg-[#0D1117] text-white">
      ...
      <div className="grid grid-cols-12 gap-4 p-4" style={{ minHeight: 'calc(100vh - 200px)' }}>
        ...
        <div className="col-span-4 bg-[#11161D] rounded-lg border border-[#232A33] p-4">
          ...
          <AssetHierarchyTree assets={assetHierarchy} onSelect={(a) => console.log('Selected:', a)} />
        </div>
        ...
        <div className="col-span-4 bg-[#11161D] rounded-lg border border-[#232A33] p-4 flex flex-col justify-between">
          ...
          <div className="mt-4">
            <div className="p-3 bg-[#1A1F2E] rounded-lg border border-[#2A3344]">
              <div className="text-xs text-gray-400 uppercase mb-2">System Health</div>
