...
export default function ExecutiveDashboard() {
  ...
  const groupByCategory = (): CategorySummary[] => {
    ...
    return categories.map((cat) => {
      ...
      const statusCounts = categoryKpis.reduce(
        (acc, k) => {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4">
          {category.kpis.map((kpi) => (
            <div
              key={kpi.kpi_id}
              className={`rounded-xl border-2 p-6 ${getStatusBgColor(kpi.status)}`}
            >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            ISO 55000 Key Objective Indicators (KOIs)
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categorySummaries.map((category) => (
          <button
            ...
            className={`rounded-xl border-2 p-6 text-left transition-all hover:shadow-lg ${getStatusBgColor(
              category.avgStatus,
            )}`}
          >
            <div className="flex items-start justify-between mb-4">
