...
export function JavisDockInteractive() {
    ...
    switch (data.type) {
      ...
      case "response": {
        ...
        if (
          ...
          (data.pending_actions as Record<string, unknown>[]).length > 0
        ) {
      ...
      case "tool_executed": {
        ...
        setPendingActions((prev) =>
          prev.filter((a) => a.id !== data.action_id),
        );
        break;
      }
      ...
      case "event": {
        ...
        const eventMessage: Message = {
          ...
          pending_actions: data.pending_action_id
            ? [
                {
                  id: data.pending_action_id as string,
                  action_type: data.event_type as string,
                  action_description: data.message as string,
                  action_payload: {},
                },
              ]
            : undefined,
  ...
  const handleSend = async () => {
    if (!input.trim() || isLoading || !ws || ws.readyState !== WebSocket.OPEN)
      return;
