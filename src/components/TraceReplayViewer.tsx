...
export function TraceReplayViewer({
  runId,
  snapshotId,
}: TraceReplayViewerProps) {
    ...
    try {
      ...
      if (snapshotId) {
        ...
        const { data: functionData, error: functionError } =
          await supabase.functions.invoke("gateway", {
            body: {
  ...
  const totalCost = snapshot.costs.reduce(
        ...
        <div className="mb-6">
          ...
          <div className="relative">
            ...
            <div className="flex justify-between text-xs text-gray-500 mt-1">
              <span>
                Step {currentStep + 1} of {snapshot.tool_calls.length}
              </span>
              <span>
                {(
                  ((currentStep + 1) / snapshot.tool_calls.length) *
                  100
                ).toFixed(0)}
                %
              </span>
