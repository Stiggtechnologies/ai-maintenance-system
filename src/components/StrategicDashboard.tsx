...
export function StrategicDashboard() {
  ...
  const fetchKPIs = async () => {
    try {
      ...
      const enrichedKPIs: KPI[] =
        kpisData?.map((kpi: any) => {
          ...
          const previousMeasurements =
            measurements?.filter((m: any) => m.kpi_id === kpi.id).slice(0, 5) ||
            [];
          let trend: "improving" | "stable" | "declining" = "stable";
          if (previousMeasurements.length >= 3) {
            const values = previousMeasurements.map(
              (m: any) => m.measured_value,
            );
            const avg1 =
              values.slice(0, 2).reduce((a: number, b: number) => a + b, 0) / 2;
            const avg2 =
              values.slice(2, 4).reduce((a: number, b: number) => a + b, 0) / 2;
      ...
      const uniqueCategories = [
        ...new Set(enrichedKPIs.map((k) => k.category_name)),
      ];
  ...
  const filteredKPIs = kpis.filter((kpi) => {
    if (selectedCategory !== "all" && kpi.category_name !== selectedCategory)
      return false;
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="mb-6">
          ...
          <p className="text-slate-400">
            Monitor all 29 ISO 55000 KPIs across your organization
          </p>
        </div>
          ...
          {filteredKPIs.map((kpi) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div
                  className={`px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(kpi.status || "red")}`}
                >
              ...
              <div className="space-y-2 mb-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-sm font-medium ${kpi.variance && kpi.variance > 0 ? "text-green-600" : "text-red-600"}`}
                  >
                    {kpi.variance
              ...
              <div className="flex items-center justify-between pt-3 border-t border-[#1A2030]">
