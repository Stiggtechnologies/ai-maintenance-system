...
import { IntelligenceRuntimePanel } from "./IntelligenceRuntimePanel";
import { IntelligenceRuntimeEnterprisePanel } from "./IntelligenceRuntimeEnterprisePanel";
...
export function CommandCenter() {
    ...
    const responseMap: Record<string, Message> = {
      default: {
        ...
        insights: {
          ...
          financial:
            "Estimated savings of $180K–$340K annually through targeted maintenance optimization.",
          actions: [
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      case "openclaw":
        return <IntelligenceRuntimePanel />;
      case "openclaw-enterprise":
        return <IntelligenceRuntimeEnterprisePanel />;
      case "billing":
        ...
        <div className="h-14 border-b border-[#232A33] flex items-center justify-between px-6 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  showToastNotification(
                    executiveMode
                      ? "Standard Mode"
                      : "Executive Intelligence Mode Activated",
                  );
                }}
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-[#161C24] border border-[#232A33] rounded-lg">
              <div
                className={`w-1.5 h-1.5 rounded-full ${sessionsRemaining > 3 ? "bg-emerald-400" : "bg-amber-400"}`}
              />
                        ...
                        {[
                          {
                            ...
                            view: "executive" as ActiveView,
                          },
                          {
                            label: "Asset Health",
                            view: "assets" as ActiveView,
                          },
                          {
                            label: "AI Analytics",
                      ...
                      >
                        {message.role === "user" ? (
                          ...
                          <div className="space-y-4">
                            <div className="text-[#E6EDF3] leading-relaxed text-sm">
                              {message.content}
                            </div>
                            {message.insights && (
                              <div className="grid gap-3 mt-4">
                                {message.insights.summary && (
                                  <div className="bg-[#161C24] border border-[#232A33] rounded-lg p-4">
                                    <h3 className="text-xs font-semibold text-[#3A8DFF] uppercase tracking-wide mb-2">
                                      Executive Summary
                                    </h3>
                                ...
                                {message.insights.actions && (
                                  <div className="bg-[#161C24] border border-[#232A33] rounded-lg p-4">
                                    ...
                                    <ul className="space-y-2">
                                      {message.insights.actions.map(
                                        (action, i) => (
                                          <li
                                            key={i}
                                            className="text-sm text-[#E6EDF3] flex gap-2"
                                          >
                                            <span className="text-[#3A8DFF] font-mono">
                                              {i + 1}.
                                            </span>
                                            {action}
                                          </li>
                                ...
                                {message.insights.confidence && (
                                  <p className="text-xs text-[#9BA7B4]">
                                    Confidence: {message.insights.confidence}
                                  </p>
                                )}
                              </div>
                            )}
          ...
          >
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <button
                  onClick={() => setShowUpgradeModal(false)}
                  className="text-[#9BA7B4] hover:text-[#E6EDF3]"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              <p className="text-[#9BA7B4] mb-6 text-sm leading-relaxed">
                SyncAI has identified measurable operational optimization
                potential. Upgrade to activate continuous monitoring and
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() => setShowUpgradeModal(false)}
                  className="w-full py-2 text-xs text-[#9BA7B4] hover:text-[#E6EDF3]"
                >
                  Maybe later
                </button>
              </div>
