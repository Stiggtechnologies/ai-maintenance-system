...
const logs: DecisionLog[] = [
  {
    ...
    action: "Created Work Order WO-20435",
    asset: "Pump P-145",
    confidence: 0.92,
    status: "executed",
    humanOverride: false,
    ...
    details:
      "Predicted bearing failure with 92% confidence. Auto-created work order for next scheduled maintenance window.",
  },
  {
    id: "2",
    timestamp: new Date(Date.now() - 1000 * 60 * 12),
    agent: "Inventory Agent",
    action: "Initiated Parts Reorder",
    asset: "Bearing SKU-129",
    confidence: 0.98,
    status: "approved",
    humanOverride: false,
    financialImpact: 12500,
    details:
      "Stock level below threshold. Automatically approved reorder based on lead time analysis.",
  },
  {
    id: "3",
    timestamp: new Date(Date.now() - 1000 * 60 * 25),
    agent: "Safety Agent",
    action: "Escalated Safety Alert",
    asset: "Conveyor C-22",
    confidence: 0.88,
    status: "executed",
    humanOverride: true,
    financialImpact: 0,
    details:
      "Safety sensor anomaly detected. Escalated to safety manager. Operator acknowledged.",
  },
  {
    id: "4",
    timestamp: new Date(Date.now() - 1000 * 60 * 45),
    agent: "Planning Agent",
    action: "Rescheduled Maintenance",
    asset: "Haul Truck #23",
    confidence: 0.85,
    status: "modified",
    humanOverride: true,
    ...
    details:
      "Originally scheduled for Friday. Human operator rescheduled to Monday due to production requirements.",
  },
  {
    id: "5",
    ...
    humanOverride: false,
    details:
      "Vibration pattern abnormal. Created inspection ticket for technician review.",
  },
  {
    id: "6",
    ...
    details:
      "Proposed increasing maintenance budget by $25K. Rejected by finance director.",
  },
  {
    id: "7",
    ...
    humanOverride: false,
    details:
      "Auto-assigned 3 work orders to available technicians based on skills and location.",
  },
  {
    id: "8",
    timestamp: new Date(Date.now() - 1000 * 60 * 150),
    agent: "Asset Management Agent",
    action: "Updated Asset Criticality",
    asset: "Compressor C-12",
    confidence: 0.89,
    status: "executed",
    humanOverride: false,
    details:
      "Updated criticality from medium to high based on production impact analysis.",
  },
];
    ...
    <div className="bg-[#1A1F2E] rounded-lg border border-[#2A3344] p-4 hover:border-[#3A4354] transition-all">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`px-2 py-0.5 rounded-sm text-xs ${config.bg} ${config.color}`}
          >
            {log.status}
          </span>
          {log.humanOverride && (
            <span className="px-2 py-0.5 rounded-sm text-xs bg-amber-400/10 text-amber-400">
              Override
            </span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        {log.financialImpact !== undefined && (
          <span
            className={
              log.financialImpact >= 0 ? "text-emerald-400" : "text-red-400"
            }
          >
            {log.financialImpact >= 0 ? "+" : ""}$
            {log.financialImpact.toLocaleString()}
          </span>
        )}
      </div>
    </div>
...
export function DecisionLogs() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "approved" | "executed" | "rejected" | "modified"
  >("all");
    ...
    <div className="min-h-screen bg-[#0D1117] text-white p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-400">
          Audit trail of all AI agent decisions and actions
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex gap-2">
          {(
            ["all", "executed", "approved", "rejected", "modified"] as const
          ).map((f) => (
            <button
              key={f}
