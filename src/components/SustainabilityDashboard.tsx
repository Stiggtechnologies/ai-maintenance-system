  ...
  return (
    <div className="bg-[#1A1F2E] rounded-lg border border-[#2A3344] p-4">
      <div className="flex items-start justify-between mb-3">
        ...
        <span
          className={`text-xs px-2 py-1 rounded-sm border ${statusColors[metric.status]}`}
        >
...
export function SustainabilityDashboard() {
  ...
  const summary = {
    environmental: metrics
      ...
      .filter((m) => m.status === "on-track").length,
    social: metrics
    ...
    <div className="min-h-screen bg-[#0D1117] text-white p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[#1A1F2E] rounded-lg border border-[#2A3344] p-4">
          ...
          <div className="text-2xl font-bold text-signal-cyan">
            {summary.social}/4
          </div>
          <div className="text-xs text-gray-500">on track</div>
        </div>
      ...
      <div className="bg-[#1A1F2E] rounded-lg border border-[#2A3344] p-4 mb-6">
          ...
          {carbonData.map((entry) => (
            ...
            >
              <div className="w-full flex gap-1 h-32 items-end">
                <div
                  className="flex-1 bg-signal-cyan/80 rounded-t"
                  style={{ height: `${(entry.scope1 / 160) * 100}%` }}
                />
                <div
                  className="flex-1 bg-emerald-500/80 rounded-t"
                  style={{ height: `${(entry.scope2 / 160) * 100}%` }}
                />
                <div
                  className="flex-1 bg-purple-500/80 rounded-t"
                  style={{ height: `${(entry.scope3 / 300) * 100}%` }}
                />
              </div>
        ...
        <div className="flex justify-center gap-6 mt-4 text-xs">
          <span className="flex items-center gap-2">
            <span className="w-3 h-3 bg-signal-cyan rounded-sm" /> Scope 1
          </span>
          <span className="flex items-center gap-2">
            <span className="w-3 h-3 bg-emerald-500 rounded-sm" /> Scope 2
          </span>
          <span className="flex items-center gap-2">
            <span className="w-3 h-3 bg-purple-500 rounded-sm" /> Scope 3
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        {(["all", "environmental", "social", "governance"] as const).map(
          (cat) => (
            <button
              key={cat}
