...
describe("PresenceBoothConversation", () => {
  ...
  it("does not send the spoken greeting as a user turn while Sync is speaking", async () => {
    renderBooth({ speaking: true });
    act(() => {
      onTranscript?.("Welcome Orville. I'm Sync, your Reliability Engineer.");
    });
    await act(async () => {
      await new Promise((resolve) =>
        setTimeout(resolve, BOOTH_UTTERANCE_SILENCE_MS + 50),
      );
    });
    expect(askBooth).not.toHaveBeenCalled();
  });

  it("sends a barged-in utterance after Sync finishes speaking", async () => {
