...
export function TacticalDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const openCount = enrichedWorkOrders.filter(
        (wo) => wo.status !== "completed",
      ).length;
      const highPriorityCount = enrichedWorkOrders.filter(
        (wo) => wo.priority === "high" && wo.status !== "completed",
      ).length;

      const mtbfMeasurement = kpiData?.find(
        (m: any) => m.kpis_kois?.kpi_code === "KPI-001",
      );
      const mttrMeasurement = kpiData?.find(
        (m: any) => m.kpis_kois?.kpi_code === "KPI-002",
      );
      const plannedMeasurement = kpiData?.find(
        (m: any) => m.kpis_kois?.kpi_code === "KOI-005",
      );
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="mb-6">
          ...
          <p className="text-slate-400">
            Manage work orders, approvals, and team performance
          </p>
        </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  {workOrders.map((wo) => (
                    <tr key={wo.id} className="hover:bg-[#0B0F14]">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium border ${getPriorityColor(wo.priority)}`}
                        >
            ...
            <div className="bg-[#11161D] rounded-xl border border-[#232A33] p-6">
              ...
              <div className="mt-4 h-2 bg-[#161C24] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((metrics.mtbf / 720) * 100, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-[#11161D] rounded-xl border border-[#232A33] p-6">
              ...
              <div className="mt-4 h-2 bg-[#161C24] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((4 / metrics.mttr) * 100, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-[#11161D] rounded-xl border border-[#232A33] p-6">
              ...
              <div className="mt-4 h-2 bg-[#161C24] rounded-full overflow-hidden">
                <div
                  className="h-full bg-teal-500"
                  style={{
                    width: `${Math.min(metrics.plannedMaintenance, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
