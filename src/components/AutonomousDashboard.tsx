...
export function AutonomousDashboard() {
  ...
  const canApprove = APPROVAL_ROLES.has(
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setNotice(
        "The dashboard could not be refreshed. Existing operating controls remain unaffected.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDecision = async (decisionId: string, approved: boolean) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke(
        "autonomous-orchestrator",
        {
          body: {
            ...
            data: { decision_id: decisionId, approved },
          },
        },
      );
      if (error || !data?.success)
        throw new Error(
          error?.message || data?.error || "Decision update failed",
        );
      setNotice(
        approved
      ...
      setNotice(
        "The decision was not changed. Please verify your approval authority and try again.",
      );
    } finally {
      setActioning(null);
    }
  ...
  const severityClass = (severity: string) => {
    ...
    return "border-signal-cyan/30 bg-signal-cyan/10 text-blue-200";
  };
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-400">
            AI recommendations remain advisory until an authorized human
            approves a governed action.
          </p>
        </div>
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-4 py-2 text-sm font-medium text-amber-300">
          Human-in-the-Loop
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {statCards.map(({ label, value, icon: Icon }) => (
          <div
            key={label}
            className="rounded-xl border border-[#232A33] bg-[#11161D] p-5"
          >
            <div className="mb-2 flex items-center justify-between">
              <Icon className="h-7 w-7 text-teal-400" />
              <span className="text-3xl font-bold text-[#E6EDF3]">{value}</span>
            </div>
      ...
      <section className="rounded-xl border border-[#232A33] bg-[#11161D] p-6">
        <div className="mb-4 flex items-center justify-between">
          ...
          {!canApprove && (
            <span className="rounded-full bg-[#161C24] px-3 py-1 text-xs text-slate-400">
        ...
        <div className="space-y-3">
          {decisions.map((decision) => (
            <article
              key={decision.id}
              className="rounded-lg border border-[#232A33] p-4 hover:border-teal-500/30"
            >
              <div className="flex items-start justify-between gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">
                        {decision.decision_data?.reason ||
                          decision.decision_data?.raw_summary ||
                  ...
                  canApprove && (
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={() => void handleDecision(decision.id, true)}
                      ...
                      <button
                        disabled={actioning === decision.id}
                        onClick={() => void handleDecision(decision.id, false)}
          ...
          {decisions.length === 0 && (
            <div className="py-12 text-center">
              <FileCheck className="mx-auto mb-3 h-12 w-12 text-slate-600" />
