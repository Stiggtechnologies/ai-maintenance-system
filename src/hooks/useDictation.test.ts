...
import { BOOTH_LISTEN_RESTART_MS } from "../lib/presence/boothListen";
...
describe("useDictation", () => {
  ...
  it("restarts on end when continuous listen is requested", () => {
    ...
    act(() => {
      vi.advanceTimersByTime(BOOTH_LISTEN_RESTART_MS);
    });
  ...
  it("surfaces start failure instead of staying silently idle", () => {
    vi.stubGlobal(
      "SpeechRecognition",
      class {
        constructor() {
          lastRecognition = new FakeRecognition();
          lastRecognition.start = vi.fn(() => {
