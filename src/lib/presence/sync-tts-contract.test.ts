...
describe("sync-tts deploy and honesty contract", () => {
  ...
  it("uses the existing OPENAI_API_KEY and does not require ElevenLabs", () => {
    ...
    expect(fn).toMatch(/ELEVENLABS_API_KEY/);
    expect(core).toContain("api.elevenlabs.io");
    expect(fn).toContain("openai");
  });

  it("records the founder-authorized AGPL vendor tree", () => {
    const notice = read("third_party/jaredrhod/NOTICE.md");
    expect(notice).toMatch(/AGPL-3\.0-or-later/);
    expect(notice).toMatch(/ai-memory-vault/);
    expect(notice).toMatch(/backtalk/);
    expect(notice).toMatch(/ai-visualizer/);
    expect(notice).toMatch(/barehands/);
    expect(notice).toMatch(/Orville Davis/);
  });
