...
describe("booth listen mode", () => {
  ...
  it("keeps listen policy Sync-owned; AGPL sources stay under third_party", () => {
    const files = [
    ...
    for (const path of files) {
      ...
      const imports = src
        ...
        .join("\n");
      expect(imports, path).not.toMatch(/third_party\/jaredrhod/i);
    }
    expect(readFileSync("third_party/jaredrhod/NOTICE.md", "utf8")).toMatch(
      /AGPL-3\.0/,
    );
  });
