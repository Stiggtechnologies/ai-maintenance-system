...
describe("navigation integrity", () => {
  ...
  it("keeps the §2 tree at 46 items in 9 groups", () => {
    ...
    expect(groupSizes).toEqual([5, 4, 5, 2, 4, 11, 7, 3, 5]);
    expect(navItems.length).toBe(46);
  });
  ...
  it("keeps the §3 role-matrix sizes after Recovery and Develop are added", () => {
    expect(allowSizes).toEqual({
      ...
      supervisor: 10,
      planner: 21,
      reliability_engineer: 32,
      maintenance_manager: 31,
      executive: 23,
      board: 7,
