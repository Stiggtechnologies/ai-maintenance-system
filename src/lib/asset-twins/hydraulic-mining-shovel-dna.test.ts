...
describe("hydraulic mining shovel DNA", () => {
  ...
  it("keeps inspection contracts governed and linked to canonical components", () => {
    const components = new Set(
      hydraulicMiningShovelTemplate.components.map(
  ...
  it("registers the template and creates a governed customer twin", () => {
    expect(getAssetClassTemplate(hydraulicMiningShovelTemplate.code)).toBe(
      hydraulicMiningShovelTemplate,
    );
    const twin = instantiateEngineeringTwin(
      hydraulicMiningShovelEngineeringDna,
      {
        assetId: "SHOVEL-001",
        siteId: "SITE-001",
  ...
  it("prohibits autonomous operational action and invented thresholds", () => {
    expect(
      hydraulicMiningShovelEngineeringDna.governance
        .autonomousOperationalActionAllowed,
    ).toBe(false);
    expect(
      hydraulicMiningShovelEngineeringDna.governance.thresholdsPolicy,
    ).toBe("approved_source_only");
