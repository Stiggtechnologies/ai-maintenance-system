...
export interface SharedComponentDnaProfile {
  ...
  /**
   * Optional bindings to canonical physics capabilities (#104).
   * New shared profiles omit this until an approved applicability mapping
   * exists for their category. When present, codes are validated against the
...
export function validateSharedComponentDna(
  ...
  const physicsByCode = new Map(
    physicsCapabilities.map((capability) => [capability.code, capability]),
  );
  ...
  for (const [profileIndex, profile] of profiles.entries()) {
    ...
    for (const [physicsIndex, physicsCode] of (
      ...
      if (!capability) {
        issues.push({
          path: physicsPath,
          message: `Unknown physics capability ${physicsCode}.`,
        });
      } else if (
        !capability.applicableSharedComponentCategories.includes(
          profile.category,
        )
      ) {
        issues.push({
