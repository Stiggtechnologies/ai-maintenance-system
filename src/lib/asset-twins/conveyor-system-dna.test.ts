...
describe("conveyor system Digital Engineering DNA", () => {
  it("keeps the canonical asset hierarchy valid", () => {
    ...
    expect(
      new Set(
        conveyorSystemTemplate.components.map((component) => component.code),
      ).size,
    ).toBe(conveyorSystemTemplate.components.length);
  });
  ...
  it("keeps DNA and shared-component references canonical and governed", () => {
    ...
    expect(conveyorSystemEngineeringDna.capabilities).toContain(
      "shared_component_composition",
    );
    expect(
      conveyorSystemEngineeringDna.sharedComponentBindings?.length,
    ).toBeGreaterThanOrEqual(7);
    expect(conveyorSystemEngineeringDna.governance.thresholdsPolicy).toBe(
      "approved_source_only",
    );
    expect(
      conveyorSystemEngineeringDna.governance
        .autonomousOperationalActionAllowed,
    ).toBe(false);
  });

  it("supports registry lookup and governed twin creation with component composition", () => {
    expect(getAssetClassTemplate(conveyorSystemTemplate.code)).toBe(
      conveyorSystemTemplate,
    );
    expect(getEngineeringDnaForAssetClass(conveyorSystemTemplate.code)).toBe(
      conveyorSystemEngineeringDna,
    );
    const twin = instantiateEngineeringTwin(conveyorSystemEngineeringDna, {
      assetId: "CV-101",
      siteId: "SITE-1",
    });
