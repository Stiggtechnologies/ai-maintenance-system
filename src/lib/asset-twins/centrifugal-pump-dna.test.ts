...
describe("centrifugal pump Digital Engineering DNA", () => {
  it("keeps the canonical hierarchy valid", () => {
    ...
    expect(
      new Set(
        centrifugalPumpTemplate.components.map((component) => component.code),
      ).size,
    ).toBe(centrifugalPumpTemplate.components.length);
  });
  ...
  it("keeps DNA, shared-component, and physics references canonical and governed", () => {
    ...
    expect(centrifugalPumpEngineeringDna.capabilities).toContain(
    ...
    expect(centrifugalPumpEngineeringDna.governance.thresholdsPolicy).toBe(
      "approved_source_only",
    );
    expect(
      centrifugalPumpEngineeringDna.governance
  ...
  it("rejects unknown physics capability references", () => {
    const invalid = {
      ...
      physicsCapabilityBindings: [
        ...(centrifugalPumpEngineeringDna.physicsCapabilityBindings ?? []),
        {
          ...
          physicsCapabilityCode: "PHYS-UNKNOWN",
          role: "invalid test binding",
        },
      ],
    };
    expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          path: "physicsCapabilityBindings[2].physicsCapabilityCode",
        }),
      ]),
    );
  });
  ...
  it("supports registry lookup and governed twin creation", () => {
    expect(getAssetClassTemplate(centrifugalPumpTemplate.code)).toBe(
      centrifugalPumpTemplate,
    );
    expect(getEngineeringDnaForAssetClass(centrifugalPumpTemplate.code)).toBe(
      centrifugalPumpEngineeringDna,
    );
    const twin = instantiateEngineeringTwin(centrifugalPumpEngineeringDna, {
      assetId: "P-101",
      siteId: "SITE-1",
    });
    ...
    expect(twin.customerOverrides.physicsCapabilityBindings).toEqual(
      centrifugalPumpEngineeringDna.physicsCapabilityBindings,
    );
  });
});
