...
describe("dragline Digital Engineering DNA", () => {
  it("keeps the dragline hierarchy and failure references canonical", () => {
    ...
    expect(
      new Set(draglineTemplate.components.map((component) => component.code))
        .size,
    ).toBe(draglineTemplate.components.length);
    expect(
      new Set(
      ...
      draglineTemplate.components.flatMap((component) => component.failureModes)
        .length,
    );
  });
  ...
  it("keeps the DNA profile canonical and approval-gated", () => {
    ...
    expect(draglineEngineeringDna.governance.engineeringApprovalRequired).toBe(
      true,
    );
    expect(
      draglineEngineeringDna.governance.customerOverridesRequireApproval,
    ).toBe(true);
    expect(
      draglineEngineeringDna.governance.autonomousOperationalActionAllowed,
    ).toBe(false);
    expect(draglineEngineeringDna.governance.thresholdsPolicy).toBe(
  ...
  it("supports canonical asset and DNA lookup", () => {
    ...
    expect(getEngineeringDnaProfile(draglineEngineeringDna.code)).toBe(
      draglineEngineeringDna,
    );
    expect(getEngineeringDnaForAssetClass(draglineTemplate.code)).toBe(
      draglineEngineeringDna,
    );
  });
