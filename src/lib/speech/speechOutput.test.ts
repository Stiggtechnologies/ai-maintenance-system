...
describe("speech output adapter", () => {
  ...
  it("falls back to speechSynthesis when cloud TTS fails or is unconfigured", async () => {
    ...
    const controller = createSpeechOutputController({
      requestCloud: vi
        .fn()
        .mockRejectedValue(new Error("cloud_tts_unconfigured")),
      playCloud: vi.fn(),
      speakBrowser,
      stopBrowser: vi.fn(),
...
describe("playAudioBlob", () => {
  ...
  it("splits spoken replies into sentences the way backtalk queues TTS", () => {
    expect(
      splitSpokenSentences("Welcome Orville. I recommend, I do not authorize."),
    ).toEqual(["Welcome Orville.", "I recommend, I do not authorize."]);
    expect(splitSpokenSentences("  Short.  ")).toEqual(["Short."]);
  });
});
