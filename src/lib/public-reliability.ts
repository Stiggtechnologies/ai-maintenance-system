...
export const PUBLIC_RELIABILITY_STORAGE_KEY = "syncai-public-reliability-v2";
export const PUBLIC_RELIABILITY_FREE_RUN_LIMIT = 1;
...
export interface PublicReliabilityResult {
...
export interface PublicReliabilityUsage {
  runCount: number;
...
export const PUBLIC_RELIABILITY_SCENARIOS: PublicReliabilityScenario[] = [
  {
    id: "open-question",
    asset: "Your asset or system",
    industry: "Asset-intensive operations",
    label: "Ask your own reliability question",
    shortLabel: "Your question",
    question:
      "Describe the asset, observed symptoms, recurrence pattern, operating context, and the decision your team needs to make.",
    signal: "Live expert assessment",
    hasReferenceData: false,
    csvText: scenarioRows["open-question"].join("\n"),
    inputs: {
      operatingHours: 1,
      failures: 1,
      repairHours: 0,
    ...
    hypotheses: [
      {
        hypothesis: "The failure mechanism is not yet evidenced",
        whyItFits: "No structured history or physical evidence has been supplied.",
        validation:
          "Capture the event sequence, operating state, alarms, inspection evidence, and as-found condition before disturbance.",
      },
      {
        hypothesis: "Operating context may be masking the recurrence pattern",
        whyItFits:
          "Load, environment, duty cycle, maintenance state, and time-to-event are not yet separated.",
        validation:
          "Compare failures against operating regime, maintenance interventions, and unaffected peer assets.",
      },
    ],
  },
  {
...
export function generatePublicReliabilityAnalysis(
  id: PublicReliabilityScenarioId,
  now = new Date(),
  question?: string,
): PublicReliabilityResult {
  const scenario = getPublicReliabilityScenario(id);
  const userQuestion = question?.trim().slice(0, 1600) || scenario.question;
...
export function parsePublicReliabilityUsage(
  raw: string | null,
): PublicReliabilityUsage | null {
  if (!raw) return null;

  try {
    const parsed = JSON.parse(raw) as Partial<PublicReliabilityUsage>;
    ...
    return {
      ...
      ? { question: parsed.question.trim().slice(0, 1600) }
        : {}),
    };
  } catch {
    return null;
...
export function readPublicReliabilityUsage(
  storage: UsageStorage,
): PublicReliabilityUsage | null {
  return parsePublicReliabilityUsage(storage.getItem(PUBLIC_RELIABILITY_STORAGE_KEY));
}

export function recordPublicReliabilityRun(
  storage: UsageStorage,
  scenarioId: PublicReliabilityScenarioId,
  completedAt = new Date(),
  question?: string,
): PublicReliabilityUsage {
  const current = readPublicReliabilityUsage(storage);
  const usage: PublicReliabilityUsage = {
    runCount: (current?.runCount ?? 0) + 1,
    scenarioId,
    completedAt: completedAt.toISOString(),
    ...(question?.trim() ? { question: question.trim().slice(0, 1600) } : {}),
  };
  storage.setItem(PUBLIC_RELIABILITY_STORAGE_KEY, JSON.stringify(usage));
  return usage;
}

export function hasUsedPublicReliabilityFreeRun(usage: PublicReliabilityUsage | null): boolean {
  return (usage?.runCount ?? 0) >= PUBLIC_RELIABILITY_FREE_RUN_LIMIT;
}
