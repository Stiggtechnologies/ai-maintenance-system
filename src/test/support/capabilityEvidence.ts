...
export function judgeFile(citation: Citation, code: CodeCorpus): Verdict {
  ...
  // existsSync/statSync and then read, which CodeQL correctly flags as a
  // time-of-check/time-of-use race: the path can change between the question and
  // the answer. Here the read IS the check, so there is no window between them.
  const expand = (root: string): string[] => {
    try {
      return readdirSync(root).map((f) => join(root, f));
    } catch {
      // Not a directory, or absent. Either way the file read below decides.
      return [root];
    }
  };
  const readIfFile = (f: string): string | null => {
    try {
      return readFileSync(f, "utf8");
    } catch {
      // Absent, a directory, or unreadable — none of which is an invoker.
      return null;
    }
  };
  const invokers = ["package.json", ".github/workflows", "supabase/config.toml"]
    .flatMap(expand)
    .filter((f) => readIfFile(f)?.includes(bare) ?? false);
  return {
    citation,
