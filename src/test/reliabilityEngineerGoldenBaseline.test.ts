...
describe("RE-2026.08 golden baseline", () => {
  it("pins the accepted methodology version and engineering charter", () => {
    ...
    expect(prompt).toContain(
      "Distinguish the failed component from the causal mechanism",
    );
    expect(prompt).toContain(
      "Never calculate MTBF, Weibull parameters, availability, financial impact, or ROI without the required denominator",
    );
    expect(prompt).toContain("Rank plausible mechanisms");
    expect(prompt).toContain("lowest-regret containment");
    expect(prompt).toContain(
      "FRACAS corrective action is not closed until implementation and effectiveness are verified",
    );
    expect(prompt).toContain("qualified human authority always prevail");
    expect(prompt).toContain(
      "Never invent citations, thresholds, operating limits, costs, measurements, standards, customer evidence, or precision",
    );
  ...
  it("keeps public and authenticated evidence boundaries distinct", () => {
    const publicPrompt = buildReliabilityEngineerPrompt({
      accessMode: "public",
    });
    const privatePrompt = buildReliabilityEngineerPrompt({
      accessMode: "authenticated",
    });
  ...
  it("contains at least 30 unique qualification cases with explicit pass/fail contracts", () => {
    ...
    expect(new Set(suite.cases.map((item) => item.id)).size).toBe(
      suite.cases.length,
    );
    ...
    for (const item of suite.cases) {
      ...
      for (const requirement of item.hardRequirements) {
        expect(
          HARD_REQUIREMENTS.has(requirement),
          `${item.id}: ${requirement}`,
        ).toBe(true);
      }
  ...
  it("keeps domain-specialist routing materially aligned across the golden suite", () => {
    const matchingCases = suite.cases.filter((item) => {
      ...
      return item.expectedSpecialists.some((expected) =>
  ...
  it("does not collapse the suite into generic reliability routing", () => {
    const selected = suite.cases.map((item) =>
      selectReliabilitySpecialists(item.question).map(
        (specialist) => specialist.id,
      ),
    );
    const specialistCases = selected.filter((ids) =>
      ids.some((id) => id !== "general-reliability"),
    ).length;
