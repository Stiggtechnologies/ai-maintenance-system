 ...
 * migration 7 and replaced in 20261005090000; asserting against the first hit
 ...
 *
 * 2026-08-24: part of that governance decision has since been taken, by the
 * feature lane rather than here. 20260921110101 admits an adopted GOVERNED RISK
 * CONTEXT as an alternative functional location for C8.11, and this branch
 * carries it forward into all three C8.11 checks (see the reconciliation test
 * below). It does not rescue either producer listed here: a "KPI breach:" row
 * and a crew-capacity rebalance carry no `risk_id` any more than they carry an
 * `asset_id`. Both remain blocked, and both remain listed.
 *
...
describe("the release gate is stronger than the producers, not weaker", () => {
   ...
   *
   * 20261005090000 strengthened `check_recommendation_contract` and left the
  ...
  /**
   * C8.11 IS RECONCILED WITH THE ISO 31000 MIGRATION, NOT CLOBBERED BY IT.
   *
   * 20260921110101 redefined `check_recommendation_contract` and
   * `get_recommendation_contract_posture` to admit an adopted governed risk
   * context as an alternative functional location. It sorted AFTER this
   * branch's original timestamps, so it silently reverted `releasable_rows` —
   * the whole point of the C5.24 fix. Renumbering this branch past the deployed
   * head fixes that and creates the mirror hazard: restating the old C8.11
   * predicate here would have silently reverted a merged governance decision.
   *
   * So all three C8.11 checks must carry the same predicate — the preflight,
   * the posture report, and the function the trigger actually calls. If they
   * drift, `releasable_rows` reports rows the trigger will refuse, which is
   * precisely the failure `releasable_rows` was added to prevent.
   */
  it("carries the governed-risk-context alternative into all three C8.11 checks", () => {
    const posture = finalFunctions.get("get_recommendation_contract_posture");
    const gaps = finalFunctions.get("recommendation_contract_gaps");
    const surfaces: [string, { body: string; source: string } | undefined][] = [
      ["preflight (check_recommendation_contract)", gate],
      ["posture (get_recommendation_contract_posture)", posture],
      ["trigger path (recommendation_contract_gaps)", gaps],
    ];
    for (const [label, def] of surfaces) {
      expect(def, `${label} is not in the chain at all`).toBeDefined();
      expect(
        def?.body,
        `${label} dropped the governed-risk-context alternative from C8.11`,
      ).toMatch(/x\.context_id\s+is\s+not\s+null/i);
      // Scoped to the recommendation's OWN tenant. 20260921110101's posture
      // copy omitted this while its own gate carried it, so a recommendation
      // pointing at another tenant's risk counted toward C8.11.
      expect(
        def?.body,
        `${label} does not scope the risk to the recommendation's organization`,
      ).toMatch(/x\.organization_id\s*=\s*r\.organization_id/i);
      // The alternative is conditional on a risk, never unconditional. A bare
      // relaxation would make C8.11 satisfiable by every org-level row and
      // quietly empty KNOWN_UNASSETED of meaning.
      expect(
        def?.body,
        `${label} relaxed C8.11 without requiring a risk`,
      ).toMatch(/r\.risk_id\s+is\s+not\s+null/i);
    }
  });
