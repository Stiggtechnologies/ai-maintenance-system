-- Industry catalog readiness: five packs become kernel_bound.
--
-- food_beverage, aviation, marine_shipping, defense and aerospace_launch each
-- shipped a template pack with no IndustryProfile behind it, so the derived
-- readiness was 'template_only' -- prose that registered as coverage in the
-- signup catalog and computed nothing. Each now declares its failure contexts
-- and binds to engines that exist, so getRiskIndustryPackCatalog() derives
-- 'kernel_bound' and this allowlist must agree or the catalog is lying to the
-- database about what it can do.
--
-- kernel_bound means the engines bind. It does NOT mean validated: every pack
-- remains validationStatus 'draft', and the claims that do not bind are named
-- in each profile's proseOnly list rather than counted as coverage.
--
-- Forward-only and idempotent: the column adds are IF NOT EXISTS and both
-- functions are CREATE OR REPLACE, so this applies cleanly whether or not
-- 20260921110103 has reached the target database.
...
begin
  ...
  case v_industry_code
    ...
    when 'food_beverage' then v_expected_label := 'Food & Beverage'; v_expected_readiness := 'kernel_bound';
    ...
    when 'aviation' then v_expected_label := 'Aviation'; v_expected_readiness := 'kernel_bound';
    when 'marine_shipping' then v_expected_label := 'Marine Shipping'; v_expected_readiness := 'kernel_bound';
    when 'data_centers' then v_expected_label := 'Data Centers'; v_expected_readiness := 'kernel_bound';
    when 'defense' then v_expected_label := 'Defense'; v_expected_readiness := 'kernel_bound';
    when 'aerospace_launch' then v_expected_label := 'Aerospace & Launch'; v_expected_readiness := 'kernel_bound';
