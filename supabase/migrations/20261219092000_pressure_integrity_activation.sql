...
declare v_org uuid:=app_current_org(); v_uid uuid; v_id bigint; v_basis text:=nullif(btrim(p_record->>'evidence_basis'),''); v_design numeric:=nullif(p_record->>'design_thickness_mm','')::numeric; v_min numeric:=nullif(p_record->>'minimum_thickness_mm','')::numeric;
begin
 ...
 if v_min is null or v_min<=0 or (v_design is not null and v_design<=0) then raise exception 'positive owner-approved minimum thickness is required; SyncAI does not invent it'; end if;
 ...
 values(v_org,btrim(p_record->>'circuit_ref'),btrim(p_record->>'description'),btrim(p_record->>'material'),btrim(p_record->>'service_fluid'),btrim(p_record->>'damage_mechanisms'),v_design,v_min) returning id into v_id;
...
begin
 ...
 if length(btrim(coalesce(p_record->>'cml_ref','')))<2 or nullif(p_record->>'measured_on','') is null or nullif(p_record->>'thickness_mm','') is null or (p_record->>'thickness_mm')::numeric<=0 or length(btrim(coalesce(p_record->>'method','')))<2 or v_basis is null or length(v_basis)<20 then raise exception 'CML, measurement date, positive thickness, method and evidence basis are required'; end if;
...
begin
 ...
 if length(btrim(coalesce(p_cml_ref,'')))<2 then raise exception 'CML reference is required'; end if;
 ...
 if v_min is null or v_min<=0 then raise exception 'positive owner-approved minimum thickness is missing; assessment is blocked'; end if;
