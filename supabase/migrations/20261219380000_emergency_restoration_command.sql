...
create table if not exists public.recovery_operating_commands (
...
create table if not exists public.recovery_operating_mode_transitions (
 id uuid primary key default gen_random_uuid(), organization_id uuid not null references public.organizations(id) on delete cascade,
...
create unique index if not exists recovery_one_pending_mode_transition on public.recovery_operating_mode_transitions(command_id) where status='pending';
create index if not exists recovery_commands_org_mode on public.recovery_operating_commands(organization_id,current_mode,updated_at desc);
create index if not exists recovery_mode_transitions_command on public.recovery_operating_mode_transitions(organization_id,command_id,requested_at desc);
...
drop policy if exists recovery_operating_commands_read on public.recovery_operating_commands;
...
drop policy if exists recovery_operating_mode_transitions_read on public.recovery_operating_mode_transitions;
...
create or replace function public.protect_recovery_operating_command() returns trigger language plpgsql set search_path=public as $$ begin
...
drop trigger if exists protect_recovery_operating_command on public.recovery_operating_commands;
...
create or replace function public.protect_recovery_mode_transition() returns trigger language plpgsql set search_path=public as $$ begin
...
drop trigger if exists protect_recovery_mode_transition on public.recovery_operating_mode_transitions;
...
create or replace function public.create_recovery_operating_command(p_command jsonb) returns jsonb language plpgsql security definer set search_path=public as $$
...
create or replace function public.request_recovery_operating_mode(p_command_id uuid,p_to_mode text,p_basis text,p_evidence_item_ids uuid[] default '{}',p_missing_evidence text[] default '{}') returns jsonb language plpgsql security definer set search_path=public as $$
...
create or replace function public.review_recovery_operating_mode(p_transition_id uuid,p_decision text,p_note text) returns jsonb language plpgsql security definer set search_path=public as $$
...
create or replace function public.get_recovery_operating_command_workspace() returns jsonb language sql stable security definer set search_path=public as $$ select case when public.app_current_org() is null then jsonb_build_object('error','forbidden') else jsonb_build_object(
...
revoke all on function public.create_recovery_operating_command(jsonb) from public,anon;
revoke all on function public.request_recovery_operating_mode(uuid,text,text,uuid[],text[]) from public,anon;
revoke all on function public.review_recovery_operating_mode(uuid,text,text) from public,anon;
revoke all on function public.get_recovery_operating_command_workspace() from public,anon;
