-- ============================================================================
-- Sync Develop Slice 1 — the Case Workspace read, extended with the §44
-- sections this slice band makes real: deliverables (D3.26), evidence
-- (D11.17), risks (D5.22), decisions + options (D3.27/D3.28), actions
-- (D11.37).
--
-- get_development_case is RE-CREATED from its 20261101090500 definition with
-- five keys appended; every pre-existing key is byte-identical (diffed at
-- authoring time) so the #282 surface renders unchanged. One call, the whole
-- position — enforced truth and displayed truth stay the same rows:
--
--   * deliverables carry their criterion linkage, so an ACCEPTED deliverable
--     renders beside the gate requirement it answers — the readiness rollup
--     consumes the acceptance state without inventing a number;
--   * risks are the case-bound rows of the ONE `risks` table; the workspace
--     rollup names OPEN High/Critical ones as gate blockers (spec workflow 2);
--   * decisions carry their full option set, the selection record (who, when,
--     rationale), evidence links and assumption links — the reconstructible
--     "why" in one readback;
--   * actions are canonical recommendations reaching the case DIRECTLY
--     (development_case_id) or VIA a case-bound risk, labelled as such, each
--     with its verification obligation state (the loop-closure debt).
--
-- SECURITY INVOKER, deliberately (the 20261101090500 argument, kept): every
-- consulted table carries an org-scoped read policy, and the risk-sensitivity
-- boundary (can_read_risk) therefore applies to this aggregate exactly as it
-- applies to the ROS surfaces — a reader who cannot see a sensitive risk does
-- not see it here either. Names resolve from user_profiles, never auth.users.
-- ============================================================================
