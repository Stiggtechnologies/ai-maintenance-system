...
begin
  ...
  -- C8.11 is satisfied by an asset OR by an adopted governed risk context.
  --
  -- The second branch is NOT this workstream's decision. 20260921110101 (Codex,
  -- ISO 31000) made it on main: "an enterprise risk treatment can be scoped to
  -- an adopted context even when no single asset is the right functional
  -- location. All other canonical release fields remain mandatory." That
  -- migration sorts before this one, so restating the C8.11 predicate here
  -- WITHOUT it would silently revert a merged governance decision — the mirror
  -- image of the revert this file exists to prevent. It is carried forward
  -- verbatim in meaning, with one repair: 20260921110101's posture copy of this
  -- predicate omitted the organization check its own gate carries, so a
  -- recommendation pointing at another tenant's risk counted. Both copies now
  -- scope the risk to the recommendation's own organization.
  --
  -- It does not admit an ORG-LEVEL recommendation: a `compute_kpi_snapshot` row
  -- has no asset AND no risk, so those stay blocked, which is what
  -- KNOWN_UNASSETED in recommendationContractProducers.test.ts records.
  if r.asset_id is not null
     or (r.risk_id is not null and exists (
           select 1 from risks x
...
-- service_role is carried forward from 20260921110101 (edge functions call the
-- preflight); anon is carried forward from the same file's hardening block.
-- Redefining the function here resets neither, but restating both keeps this
-- file's final state readable without cross-referencing two migrations.
revoke execute on function public.check_recommendation_contract(uuid) from public, anon;
grant execute on function public.check_recommendation_contract(uuid) to authenticated, service_role;
...
grant execute on function public.get_recommendation_contract_posture() to authenticated, service_role;
  ...
  for v_org in select id from organizations loop
    ...
    for k in select * from kpi_catalog where computable loop
      ...
      case k.kpi_key
        ...
        -- computed. 20261005090200 marks both non-computable and says why;
