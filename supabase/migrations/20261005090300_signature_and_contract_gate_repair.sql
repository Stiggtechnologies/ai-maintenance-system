...
--
-- An adversarial review of 20261005090000 and 20261005090100 against a real
...
-- 20261005090000 announced is not running at all.
--
...
--    20261005090000 added `contract_field_blank` and `contract_narrative_blank`
...
-- contract_field_blank/contract_narrative_blank from 20261005090000,
...
--
-- Same field set, same messages, same order as 20260829093000, with two
-- changes and no others:
--
--   a. what counts as PRESENT — sentinels and sub-24-character narratives no
--      longer do, which is the change 20261005090000 claimed to have made and
--      made only to the preflight;
--   b. C8.11 admits an adopted governed risk context, carried forward from
--      20260921110101 so the enforcing path agrees with the preflight. Without
--      (b) the posture's `releasable_rows` would count rows this function then
--      refuses.
-- ---------------------------------------------------------------------------
create or replace function public.recommendation_contract_gaps(r recommendations)
returns text[]
-- `stable`, not `immutable`: C8.11 now reads `risks` (see below), and a function
-- that reads a table is not immutable. Nothing indexes or generates on this
-- function, so the volatility change costs only a planner optimisation.
language sql stable set search_path = public as $$
  select array_remove(array[
    -- Identical predicate to check_recommendation_contract() in
    -- 20261005090000, which carries 20260921110101's governed-risk-context
    -- alternative. The preflight and the trigger must agree on C8.11 or
    -- `releasable_rows` reports rows the trigger will refuse.
    --
    -- This function is SECURITY INVOKER and `risks` is under RLS, so a caller
    -- who cannot see the risk gets `false` and C8.11 reads missing. That is
    -- fail-CLOSED, and unreachable in practice: a caller who cannot see the
    -- risk cannot see the recommendation either.
