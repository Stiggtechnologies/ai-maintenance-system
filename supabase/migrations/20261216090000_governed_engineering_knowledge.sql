...
-- Governed attachment is opt-in. C2.15 `kb_ingest_document` continues to write
-- tenant chunks without a governed source; those stay on the existing
-- claim-type/document-class standing rules. Attaching a governed source is
-- the stricter path and then requires checksum + provenance.
...
begin
  ...
  if nullif(trim(coalesce(new.content_checksum, '')), '') is null then
    raise exception 'Governed tenant knowledge requires a content checksum';
  end if;
  if new.provenance is null or new.provenance = '{}'::jsonb then
    raise exception 'Governed tenant knowledge requires provenance metadata';
  end if;
...
begin
  ...
  -- change a limit, or execute an operational action.
...
declare
  v_q tsquery;
  -- Gate on the SESSION, not the profile. The Invariant-lane regex requires
  -- `app_current_org()` un-schema-qualified; `public.app_current_org()` fails it.
