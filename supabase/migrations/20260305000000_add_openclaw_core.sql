...
CREATE INDEX IF NOT EXISTS idx_openclaw_memory_embedding ON openclaw_memory USING hnsw (embedding vector_cosine_ops);
...
-- Basic policies (tenant isolation via user_profiles id)
...
CREATE POLICY "openclaw_messages_select" ON openclaw_messages FOR SELECT TO authenticated
  USING (session_id IN (SELECT id FROM openclaw_sessions WHERE tenant_id IN (SELECT id FROM user_profiles WHERE id = auth.uid())));
CREATE POLICY "openclaw_messages_insert" ON openclaw_messages FOR INSERT TO authenticated
  WITH CHECK (session_id IN (SELECT id FROM openclaw_sessions WHERE tenant_id IN (SELECT id FROM user_profiles WHERE id = auth.uid())));
...
CREATE POLICY "openclaw_event_runs_select" ON openclaw_event_runs FOR SELECT TO authenticated
  USING (event_id IN (SELECT id FROM openclaw_events WHERE tenant_id IN (SELECT id FROM user_profiles WHERE id = auth.uid())));
CREATE POLICY "openclaw_event_runs_insert" ON openclaw_event_runs FOR INSERT TO authenticated
  WITH CHECK (event_id IN (SELECT id FROM openclaw_events WHERE tenant_id IN (SELECT id FROM user_profiles WHERE id = auth.uid())));
