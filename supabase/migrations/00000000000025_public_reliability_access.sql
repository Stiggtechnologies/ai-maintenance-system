-- Anonymous free-access abuse throttling. This table never stores prompts,
-- customer data, IP addresses, or user-agent strings.
create table if not exists public.public_reliability_rate_limits (
...
alter table public.public_reliability_rate_limits enable row level security;

create or replace function public.consume_public_reliability_allowance(
...
revoke all on public.public_reliability_rate_limits from public, anon, authenticated;
revoke all on function public.consume_public_reliability_allowance(text, timestamptz, integer) from public, anon, authenticated;
grant execute on function public.consume_public_reliability_allowance(text, timestamptz, integer) to service_role;
