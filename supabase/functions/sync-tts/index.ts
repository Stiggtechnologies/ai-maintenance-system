...
Deno.serve(async (request) => {
  ...
  if (request.method === "GET") {
    const eleven = isElevenLabsConfigured(ELEVENLABS_API_KEY, ELEVENLABS_VOICE_ID);
    const openai = isCloudTtsConfigured(OPENAI_API_KEY);
    const configured = eleven || openai;
    return json({
      configured,
      engine: eleven ? "elevenlabs" : openai ? "openai" : "unconfigured",
    });
  }
