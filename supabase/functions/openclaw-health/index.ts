...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { error: dbError } = await supabase.from('openclaw_agents').select('id').limit(1);
    checks.push({ name: 'database', status: dbError ? 'error' : 'ok', details: dbError ? dbError.message : {} });

    const { error: memoryError } = await supabase.from('openclaw_memory').select('id').limit(1);
    checks.push({ name: 'memory_store', status: memoryError ? 'warn' : 'ok', details: memoryError ? memoryError.message : {} });

    for (const c of checks) {
      await supabase.from('openclaw_health_history').insert({ name: c.name, status: c.status, details: c.details || {} });
    }
