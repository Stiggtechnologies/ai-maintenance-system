...
export function isElevenLabsConfigured(
  ...
  return Boolean(apiKey && apiKey.trim() && voiceId && voiceId.trim());
}
...
export async function synthesizeElevenLabsSpeech(input: {
  ...
  const response = await fetchImpl(
    `${ELEVENLABS_SPEECH_URL}/${encodeURIComponent(input.voiceId)}`,
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        text: input.text,
        model_id: input.model ?? DEFAULT_ELEVENLABS_MODEL,
      }),
    },
  );
  if (!response.ok) {
  ...
  return {
    ...
    model: input.model ?? DEFAULT_ELEVENLABS_MODEL,
  };
}
