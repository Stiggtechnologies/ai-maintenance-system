// recovery-activation-pull — a narrow, user-triggered, read-only JSON adapter.
//
// The endpoint, entity and mapping all come from tenant-scoped approved
// database configuration. The caller cannot submit an arbitrary URL. Endpoint
// hosts must also be present in RECOVERY_CONNECTOR_ALLOWED_HOSTS, which closes
// the SSRF boundary independently of the tenant configuration. Credentials are
// resolved by opaque URI from RECOVERY_CONNECTOR_CREDENTIALS_JSON and are never
// stored in or returned by the database.
...
function safeLog(event: string, details: Record<string, unknown> = {}): void {
  console.log(
    JSON.stringify({ fn: "recovery-activation-pull", event, ...details }),
  );
}
...
async function fetchJson(
  ...
  try {
    ...
    if (declared > MAX_RESPONSE_BYTES)
      throw new Error("Source response exceeds the 10 MB activation limit.");
...
Deno.serve(async (request) => {
  ...
  try {
    const source = await rpc<{
      ...
      constants: Record<string, unknown>;
    }>(client, "get_recovery_activation_source", {
    ...
    if (source.system_kind === "file")
      throw new Error(
        "Manual file sources run through the in-product CSV path.",
      );
    ...
    if (dryRun) {
      ...
      for (let index = 0; index < rows.length; index += 500) {
        const preview = await rpc<{
          ...
          rejected: number;
          results: unknown[];
        }>(client, "preview_recovery_activation_batch", {
          p_connector_key: connectorKey,
          p_entity_type: entityType,
    ...
    for (let index = 0; index < rows.length; index += 500) {
      const batch = await rpc<typeof totals>(
        client,
        "ingest_recovery_activation_batch",
        {
          p_run_id: runId,
          p_rows: rows.slice(index, index + 500),
        },
    ...
    if (runId) {
      try {
        await rpc(client, "finish_connector_run", {
          ...
          p_status: "failed",
          p_error:
            "Recovery activation adapter failed; inspect Edge Function logs.",
        });
      } catch {
