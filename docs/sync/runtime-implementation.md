...
- Voice output prefers the signed-in `sync-tts` edge function (OpenAI Speech via existing `OPENAI_API_KEY`, optional ElevenLabs overlay) and falls back to browser `speechSynthesis`. CopilotDock still gates playback with `sync_voice_output`; Meet Sync speaks when unmuted.
