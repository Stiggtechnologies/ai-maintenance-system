...
- Live conversation uses the signed-in OpenAI Realtime session through the governed Sync tools. Non-Realtime playback prefers the signed-in `sync-tts` edge function and falls back to browser `speechSynthesis`. The legacy Meet Sync booth is no longer mounted in AppShell; the welcome strip keeps only its greeting, KPI brief, and mute controls.
...
4. **Meeting capture:** meeting mode provides governed facilitation semantics; it does not yet provide authenticated multi-speaker diarization, calendar joining, or a dedicated meeting-outcome persistence model. The legacy Meet Sync booth and its `presence_meeting_vault` continuity remain retained for recovery but are not mounted in AppShell; they are not the global meeting-mode surface, a Decision Case record, or authorization.
