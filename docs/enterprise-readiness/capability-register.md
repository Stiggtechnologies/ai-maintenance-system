...
| C2.03 | Failure modes, cause codes, damage mechanisms, corrective actions                | 🟡 26 damage mechanisms shipped as reference data alongside the adopted taxonomy. NOTE: the real fleet's actual_failure_mode field holds SYSTEM GROUPS ('Engine Group', 'Hydraulic System'), not mechanisms — mechanism-level analysis needs that field recoded           |
...
| C2.17 | Inventory-system integration                                                     | 🟡 the ingestion contract exists — material_events carries source_system and every table takes external identifiers — but no live connector is wired yet                                                                                                                  |
...
never changed, only the count of it). Current tally: ✅ 81 · 🟡 99 · ❌ 217. _(2026-08-07: reconciled after parallel merges — C7.01/03/04/11 reliability engine, C4.13–17 + C6.22 closed-loop tail, C3.01–12 taxonomy, C5.04 scheduler all verified present on main.)_
