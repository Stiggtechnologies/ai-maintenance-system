...
| C2.03 | Failure modes, cause codes, damage mechanisms, corrective actions                | 🟡 the source vocabulary is now classified rather than mislabelled: 67 values → 44 system groups (4,035 WOs), 15 activity types (1,654), 8 delay reasons (311), 0 unclassified. work_orders gains system_group/work_activity/delay_reason beside the untouched raw label, plus a human-coded failure_mechanism_id with candidate shortlists per system group. Mechanism coding is 1/4,330 — the platform never infers a mechanism from a system group, so this figure is what people have actually determined (migration 20260810200000) |
| C2.04 | Process historian, alarms, trips, operating context                              | ❌                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| C2.05 | Condition-monitoring readings and diagnostic reports                             | ✅ condition_readings time series (the sensors table stored only last_value, so no history existed) + edge-triggered condition_alerts + 18 detection techniques, 26 damage mechanisms and an 88-pair detectability matrix (migrations 20260810090000/20260810120000)                                                                                                                                                                                                                                                                     |
...
| C6.15 | Work health: waiting-on-material time                                          | ✅ mean hours from material request to reservation/kitting/issue, computed from the material_events stream — verified at 30.0 h on a seeded lifecycle. The event stream, not a status column, is what makes an interval measurable                                                                                                                                                          |
...
| C6.26 | Segmentation by asset class, criticality, site, operating regime, failure mode | 🟡 get_segmented_reliability over asset class / criticality / site / system group / MECHANISM. The old 'failure_mode' dimension was segmenting by system group while calling it failure mode; it is now an alias that reports dimension='system_group' honestly, and a true mechanism axis reads human-coded rows ONLY. Operating-regime segmentation awaits operating-context data (C2.04) |
...
never changed, only the count of it). Current tally: ✅ 81 · 🟡 105 · ❌ 211. _(2026-08-07: reconciled after parallel merges — C7.01/03/04/11 reliability engine, C4.13–17 + C6.22 closed-loop tail, C3.01–12 taxonomy, C5.04 scheduler all verified present on main.)_
