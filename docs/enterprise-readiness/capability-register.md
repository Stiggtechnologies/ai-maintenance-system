...
| U21.02 | Self-serve fleet-history onboarding and autonomous asset onboarding                                                                                                                               | 🟡 import wizard proposes mapping/vocabulary/preview for human confirmation (src/lib/fleet-import, 18 tests), and every one of the 146 onboarding requirements now carries a PROVENANCE LADDER — connector → derive → library/AI → human — resolved by run_onboarding_resolution. On the auxiliary fleet this cut apparent human effort from 65.5 to a true floor of 17.0 irreducible items per asset, with 3,029 more named against the specific connection that would clear them (CMMS 1,157, historian 1,008, documents 720). CSV only; .xlsx needs a parsing dependency |
...
never changed, only the count of it). Current tally: ✅ 90 · 🟡 110 · ❌ 199. _(2026-08-07: reconciled after parallel merges — C7.01/03/04/11 reliability engine, C4.13–17 + C6.22 closed-loop tail, C3.01–12 taxonomy, C5.04 scheduler all verified present on main.)_
