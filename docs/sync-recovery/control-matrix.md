...
**2026-09-25 — FOUR GOVERNED ACTIONS WIRED.** Recurrence classification,
cannibalization proposal, economic-assumption authoring and site work-zone
relationship approval now have named-human controls on the Recovery workspace.
**31 of the 39 close-out functions are now reachable** through the product
service/surface; an exact source-contract test prevents that count from
drifting silently. The remaining 8 are deliberately internal triggers/helpers
(`recovery_energy_rank`, `enforce_recovery_energy_state`, `recovery_hash_uniform`,
`run_recovery_escalation_clock`, and `refresh_restoration_readiness`, which
stays an internal step of `refresh_recovery_planning_inputs`) or connector-side
master-data writers (`record_asset_meter_reading`, `record_component_installation`,
`record_component_removal`). A cannibalization proposal still creates a pending
approval and performs no transfer. Economic figures are human-entered and do
not verify value. Zone adjacency is still recorded by a human, not computed.

**2026-08-24 — UPDATED FOR PRODUCT WIRING.** Control, Optimize and Learn are
real tabs in the Recovery workspace. The earlier count of 27 reachable
functions is superseded by the paragraph above.
...
|   4 | Physical work-zone conflict modeling  | Contract shipped; still human-declared              | `work_zone_relationships` + a fail-closed interference constraint; CI proves parallel work is refused until a verified separation exists. Control now records that declaration through `set_work_zone_relationship` (named person, basis required, no spatial computation, no work start). The automated spatial/interference model still does not exist. |
...
|   7 | Rework / first-time-right restoration | Partial                                             | Learn renders FTR metrics, refreshes recurrence candidates, and classifies one candidate through `classify_recovery_recurrence` (confirmed or rejected, basis required). Refresh does not confirm a recurrence. Only a human-confirmed verdict removes the event from first-time-right.                                                                   |
...
|  10 | Cannibalization / substitution        | Partial                                             | Optimize renders approved alternates and donor candidates and can propose one listed donor through `propose_recovery_cannibalization`. The proposal creates a pending canonical approval and performs no transfer. A donor that is not in the listed options cannot be typed in by id.                                                                    |
...
|  22 | Richer economic decision rules        | Partial                                             | Learn records human-entered labour/overtime/contractor/logistics/risk/life-cycle assumptions through `set_recovery_economic_assumptions` (blank is not zero) and renders the governed result. Those figures do not verify value; `verify_value_metric()` remains the only authority that can.                                                             |
...
- **31 of 39 close-out functions are product-reachable**; the other 8 are
  internal helpers/triggers or connector-side master-data writers, named in
  the header.
