...
- **A multi-file floor edit.** Lowering a threshold in `scripts/reliability-baseline-floor.mjs` _and_ in the ratchet test's independent literals passes every check. That is the intended residual: it survives on review visibility, and every hunk in that diff is visibly about lowering a safety floor. It was made more expensive rather than impossible — dropping one entrypoint used to silently evict everything only that entrypoint reached (one deleted line de-protecting five files), so `MINIMUM_PROTECTED_PATH_COUNT` now catches the eviction as well as the deletion. Deleting the guard outright takes four coordinated edits, one of which fails all 117 test files.
