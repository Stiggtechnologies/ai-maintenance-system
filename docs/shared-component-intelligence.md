...
- Existing `COMP-DNA-BEARING-ROLLING` and `COMP-DNA-COUPLING-FLEXIBLE` codes
  stay stable. #104 attached physics bindings and bumped those two profiles
  to `schemaVersion` `0.2.0`. `COMP-DNA-SEAL-MECHANICAL` and
  `COMP-DNA-LUBRICATION-SYSTEM` remain `0.1.0`.
- `physicsCapabilityCodes?` is implemented by #104. Bearing and coupling
  keep those bindings. Newly added shared profiles do not attach physics
  codes — current physics capabilities only apply to `bearing`, `coupling`,
  `shaft`, and `gear_stage`. `validateSharedComponentDna` consults the
  physics library for unknown, duplicate, and inapplicable codes.
