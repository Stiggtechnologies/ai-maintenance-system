...
test "$(psqlc "select count(*) from jsonb_array_elements(sync_spec34_edges()) x where x->>'status'='absent'")" = "4"
test "$(psqlc "select (sync_spec34_absent_edge_audit()->>'absentEdgeCount')")" = "4"
# ZERO newly closable: every absence claim still holds against the catalogue.
test "$(psqlc "select (sync_spec34_absent_edge_audit()->>'newlyClosableCount')")" = "0"
# THE ONE THAT CLOSED. 5C recorded it as absent from prose nothing checked.
...
# ...and the column it names really is there, which is what made it closable.
...
# The four that did not close each name the column that would close them.
test "$(psqlc "select count(*) from jsonb_array_elements(sync_spec34_absent_edge_audit()->'edges') x where x->>'closesWhen' is not null")" = "4"
...
test "$(jqp "$R" "x['graph']['absentEdgeCount']")" = "4"
...
echo "one of §34's five absent edges honestly closed and the other four checked."
