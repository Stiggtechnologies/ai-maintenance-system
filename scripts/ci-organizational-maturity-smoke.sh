...
read -r AUTHOR_ID ADMIN_ID <<<"$(PGPASSWORD=postgres psql -h 127.0.0.1 -p 54322 -U postgres -d postgres -qAt -F ' ' -v ON_ERROR_STOP=1 -c "select (select id from user_profiles where organization_id='$ORG' and email='demo@syncai.ca'),(select id from user_profiles where organization_id='$ORG' and email='admin@syncai.ca')")"
test -n "$AUTHOR_ID" && test -n "$ADMIN_ID"
...
INCOMPLETE=$(rpc "$AUTHOR" record_organizational_maturity_assessment '{"p_assessment":{"title":"U22 incomplete","scope":"Acceptance test scope","evidenceSummary":"Incomplete input must refuse explicitly.","domains":[]}}')
...
UNVERIFIED=$(rpc "$AUTHOR" record_organizational_maturity_assessment "$UNVERIFIED_PAYLOAD")
err "$UNVERIFIED" 'requires same-tenant verified canonical evidence'

VERIFY=$(rpc "$ADMIN" verify_evidence_item "{\"p_evidence_id\":\"$EVIDENCE\",\"p_method\":\"Independent U22 evidence review\",\"p_outcome\":\"verified\",\"p_note\":\"The supplied record is fit to support the organizational maturity acceptance test.\"}")
ok "$VERIFY"
...
RECORDED=$(rpc "$AUTHOR" record_organizational_maturity_assessment "$ASSESSMENT_PAYLOAD")
ok "$RECORDED"
...
values('$OTHER_ORG','Foreign maturity','A foreign tenant assessment','Foreign tenant evidence basis for the isolation test.',2,2,'$AUTHOR_ID') returning id;
SQL
)
test -n "$FOREIGN_ID"
FOREIGN_REVIEW=$(rpc "$ADMIN" review_organizational_maturity_assessment "{\"p_assessment_id\":\"$FOREIGN_ID\",\"p_decision\":\"approved\",\"p_review_note\":\"A foreign-tenant record must remain invisible to this reviewer.\"}")
err "$FOREIGN_REVIEW" 'assessment not found in this organization'
SELF_REVIEW=$(rpc "$AUTHOR" review_organizational_maturity_assessment "{\"p_assessment_id\":\"$ASSESSMENT_ID\",\"p_decision\":\"approved\",\"p_review_note\":\"The original assessor cannot independently approve this record.\"}")
err "$SELF_REVIEW" 'assessor cannot independently review'

PGPASSWORD=postgres psql -h 127.0.0.1 -p 54322 -U postgres -d postgres -qAt -v ON_ERROR_STOP=1 <<SQL
do \$\$ begin
  begin update organizational_maturity_assessments set status='approved',reviewed_by='$ADMIN_ID',reviewed_at=now(),review_note='Direct mutation must be refused even when all fields are supplied.' where id='$ASSESSMENT_ID';
...
APPROVED=$(rpc "$ADMIN" review_organizational_maturity_assessment "{\"p_assessment_id\":\"$ASSESSMENT_ID\",\"p_decision\":\"approved\",\"p_review_note\":\"Independent review confirms the complete evidence-backed maturity baseline.\"}")
ok "$APPROVED"
PGPASSWORD=postgres psql -h 127.0.0.1 -p 54322 -U postgres -d postgres -qAt -v ON_ERROR_STOP=1 <<SQL
do \$\$ begin
...
SQL
WORKSPACE=$(rpc "$ADMIN" get_organizational_maturity_workspace '{}')
ok "$WORKSPACE"
