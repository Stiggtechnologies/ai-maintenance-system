...
ORG='11111111-1111-1111-1111-111111111111'; OTHER='99999999-9999-9999-9999-999999999916'; OTHER_SITE='98600000-0000-0000-0000-000000000002'; FOREIGN_COMMAND='98600000-0000-0000-0000-000000000012'; EVIDENCE='98600000-0000-0000-0000-000000000021'; EVENT='98600000-0000-0000-0000-000000000031'
...
psqlc "insert into sites(id,organization_id,name) values('$OTHER_SITE','$OTHER','U16 foreign site') on conflict(id) do nothing"
psqlc "insert into recovery_operating_commands(id,organization_id,site_id,command_ref,created_by) values('$FOREIGN_COMMAND','$OTHER','$OTHER_SITE','U16-FOREIGN',(select id from auth.users where email='demo@syncai.ca')) on conflict(id) do nothing"
...
FOREIGN=$(rpc "$AUTHOR" request_recovery_operating_mode "{\"p_command_id\":\"$FOREIGN_COMMAND\",\"p_to_mode\":\"elevated_risk\",\"p_basis\":\"Cross tenant command must remain invisible and refused.\"}"); err "$FOREIGN" 'operating command not found'
...
test "$(psqlc "select count(*) from audit_events where organization_id='$ORG' and entity_type like 'recovery_operating_mode%' and event_data->>'command_id'='$COMMAND'")" = 18
