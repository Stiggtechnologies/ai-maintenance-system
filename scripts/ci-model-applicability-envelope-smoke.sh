...
: "${API_URL:?}" "${ANON_KEY:?}"
ORG='11111111-1111-1111-1111-111111111111'; OTHER_ORG='99999999-9999-9999-9999-999999999919'
# RFC4122 version/variant required by enforce_model_applicability_on_calculation.
EVIDENCE='98190000-0000-4000-8000-000000000001'
...
insert into model_register(organization_id,model_key,version,model_kind,purpose,human_in_loop,is_engineering_model,lifecycle_state,production_eligible,manifest,manifest_checksum,applicability_envelope,verification_contract,required_reviewer_role_key,source_rights_confirmed,runtime_mode,calculation_key,author_id,limitations,training_data,validation_summary,applicability_summary,approval_status,submitted_by,submitted_at)
values('$ORG','ci.u19.applicability','1.0.0','deterministic_physics','U19 acceptance model',true,true,'engineering_approved',false,'{}',repeat('0',64),'{"assetFamilies":["pump"]}','{}','model_reviewer_vibration_and_rotordynamics',true,'allowlisted_deterministic','ci_u19_calculation','$AUTHOR_ID','Screening only; no operating or maintenance authority is granted.','{"status":"not_applicable","basis":"This deterministic method has no trained statistical population."}','{"status":"recorded","basis":"Independent U19 envelope evidence review is bound to this version."}','{"status":"recorded","basis":"U19 acceptance envelope covers pump duty and environment classes."}','pending_review','$AUTHOR_ID',now()) returning id;
SQL
); test -n "$MODEL_ID"
...
REGISTRY=$(rpc "$ADMIN" review_model_registry_version "{\"p_model_register_id\":$MODEL_ID,\"p_decision\":\"approved\",\"p_evidence_item_id\":\"$EVIDENCE\",\"p_review_basis\":\"Independent registry review confirms this exact U19 version and its bounded acceptance use.\"}"); ok "$REGISTRY"
PGPASSWORD=postgres psql -h 127.0.0.1 -p 54322 -U postgres -d postgres -qAt -v ON_ERROR_STOP=1 <<SQL
update model_register set production_eligible=true,lifecycle_state='production_eligible' where id=$MODEL_ID;
...
echo "U19 ledger counts refused_duty|computed|envelope_approval|review_audit=$COUNTS"
test "$COUNTS" = '1|1|1|1'
