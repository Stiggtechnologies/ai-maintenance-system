...
test "$(echo "$BEFORE" | python3 -c "import sys,json; d=json.load(sys.stdin); print(sum(1 for x in d if x.get('chunk_id')=='SYNTH-PUMP-MANUAL-R1-001'))")" = "0"
...
# C2.15 ungoverned tenant intake must remain retrievable. The governed-source
# overlay is opt-in; requiring a source on every tenant chunk would close the
# live Knowledge Base ingest door.
C215=$(curl -fsS -X POST "$API_URL/rest/v1/rpc/kb_ingest_document" \
  -H "apikey: $ANON_KEY" -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"p_source_id":"c215-compat-alphaqz","p_title":"C2.15 compat intake","p_document_class":"client_supplied","p_chunks":[{"chunk_index":1,"content":"c215compatalphaqz ungoverned tenant intake must remain retrievable after the governed-source overlay"}]}')
echo "$C215" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d.get('status')=='indexed', d"
C215_HITS=$(curl -fsS -X POST "$API_URL/rest/v1/rpc/retrieve_kb_context" \
  -H "apikey: $ANON_KEY" -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"p_query":"c215compatalphaqz","p_claim_type":"failure_behaviour","p_limit":10}')
test "$(echo "$C215_HITS" | python3 -c "import sys,json; d=json.load(sys.stdin); print(sum(1 for x in d if 'c215compatalphaqz' in (x.get('content') or '')))")" = "1"

echo "Governed knowledge smoke passed: draft hidden, approved source retrieved, cross-tenant denied, AI authority refused, C2.15 ungoverned intake still retrieved."
