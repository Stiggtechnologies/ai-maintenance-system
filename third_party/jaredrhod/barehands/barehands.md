...
```json
{
  ...
  "orbs": [
    {
      "title": "Notes",
      "path": "/absolute/path/to/their/vault",
      "kind": "notes"
    },
...
```json
{
  "hooks": {
    "UserPromptSubmit": [
      {
        "hooks": [
          { "type": "command", "command": "printf thinking > REPO/state/state" }
        ]
      }
    ],
    "Stop": [
      {
        "hooks": [
          { "type": "command", "command": "printf idle > REPO/state/state" }
        ]
      }
    ]
  }
...
1. **They want ONE more piece and nothing else.** Fastest route: say the sentence to you, right here, right now. Each repo installs from one line, for example _"clone https://github.com/jaredrhod/backtalk.git, then read backtalk/backtalk.md and set me up."_ You do it in this session and they are done.
