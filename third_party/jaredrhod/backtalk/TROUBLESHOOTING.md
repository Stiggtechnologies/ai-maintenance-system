...
- **Mic permission never appeared / recording is silent**: launch from a normal terminal window, not a background service or launcher daemon: the process inherits the _terminal's_ microphone permission. Check the input device: `python -m sounddevice` lists them.
