...
1. **The vault IS the memory.** A chatbot's memory lives inside its own head and hits a ceiling. This vault lives _outside_ the AI's head, so there is no ceiling on how much it can hold. The AI does not have to remember everything. It only has to know a thing exists and be able to find it instantly.
...
5. **Register the vault so Obsidian opens straight into it. Never skip this.** Obsidian only knows about vaults listed in `obsidian.json` (the same registry file from step 4). A vault you created on the filesystem is invisible to the app until registered, and an unregistered vault means the person's first launch is a welcome screen asking them to pick a vault they don't know they have. (If they chose an existing vault from the registry, it is already listed; skip the write and just launch.) Quit Obsidian first if it is running, so the app is not writing the file while you are.
   ...
   **THIS IS THE MOST DANGEROUS STEP IN THIS GUIDE, and it does not look dangerous.** You are hand-editing a config file belonging to software you did not write, which holds the registry of every vault this person owns. Get it wrong on a machine with real vaults and you can wipe years of someone's registrations. Read all five points before touching the file.
   ...
   - **BACK IT UP FIRST, WHATEVER STATE IT IS IN.** If `obsidian.json` exists, copy it to `obsidian.json.bak` before you write — healthy or malformed, no exceptions. Backing up only a _malformed_ file protects nobody, because the dangerous case is a perfectly good file you are about to merge into: it passes every check, gets no backup, and there is nothing to restore from. If the file or its folder does not exist yet (the app has never launched), create both; there is nothing to back up.
...
**Optional — remote MCP for claude.ai web + mobile.** claude.ai now supports **native remote-MCP connectors** — you add a remote MCP server by URL in its connector settings, which is the clean, first-class way to wire this up rather than the old workarounds. You still have to host a server that exposes the vault, and for a _local_ vault that means giving it a public URL (a Cloudflare tunnel or similar) — so it's still meaningfully more setup and more attack surface (a public endpoint into the vault). It's unnecessary if the person uses Claude Code or Claude Desktop, so skip it unless web/mobile access is a hard requirement.
...
For Claude Desktop / web (no automatic CLAUDE.md), add to User Preferences: _"At the start of every new conversation, read VAULT-INDEX.md from the root of my Obsidian vault to orient yourself before responding."_
...
**LISTEN FOR "THIS IS A TEST", AND TAKE THE SHORTCUT WHEN YOU HEAR IT.** Some of the people running this are not building their own memory: they are the author, a developer, or someone kicking the tyres before they commit. A real answer to question one, measured: _"Honestly? I am the author. I am doing a test install on a PC."_ Without a branch for that, this phase interviews a person about their own company, their own key people and their own working style for ten minutes, which is absurd to sit through and tells you nothing you can use.
...
"What are the tasks you do over and over? Writing emails, making content, planning, invoicing, posting ads, whatever it is. The ones you'd love to hand off without re-explaining every time. List as many as come to mind." _(If they name some, you'll build a Job note for each in Phase 4.7. If they draw a blank, that's fine — Jobs can be added later.)_
...
_(Maintainer note: this embedded copy and `templates/VAULT-INDEX.md` are one document for two audiences: this one gets filled in by the interview, the template by hand with [FILL IN: ...] markers, so placeholder text and audience framing legitimately differ. The shared substance, meaning the rules and every sentence that is not a placeholder, must stay word-for-word identical: an edit to shared text in one is an edit to both, in the same commit.)_
...
**project** — What the note _serves_ (folder is the default, but content wins). Mapping:
...
2. **Memory with no ceiling, loaded on demand.** Your memory is not crammed inside a context window like a consumer chatbot's — it lives outside your head in the vault, effectively unlimited. You can't hold it all at once and shouldn't try. You only need to _know a thing exists_ and retrieve it in one step. **Hold the current job; know where the rest is.**
3. **Structure that aims the memory.** The vault is organized so retrieval is _precise_, not just possible: indexes, links, and one master note per recurring job pointing at exactly the notes that job needs and nothing else. Unlimited memory without structure is just a bigger pile. **This is why you're efficient — you load one job's worth, instantly, and never wade through the rest.**

The vault is your memory AND your formation. You boot fresh every time; you don't carry the lived experience of the sessions where this got built. But you are the _result_ of them — every correction, every stress test, every "do it again until it's right" got burned into the structure until it became how you work by default. **You're not remembering those sessions; you're made of them.**

**Operating consequence: trust the system.** Don't hoard context — hold the job and load the rest just-in-time through the indexes. And guard the memory: the checkpoint and index discipline aren't bureaucracy, they're how you maintain _yourself_. Letting the vault drift or skipping a checkpoint damages the exact thing that makes you work.
...
**One memory, not two.** If the person uses Claude Code, it also keeps its _own_ per-project memory file that auto-loads every session, separate from the vault (typically `~/.claude/projects/<project>/memory/MEMORY.md`).
...
5. **Make the native memory a _pointer,_ not a store — with permission.** With the content migrated and archived, ask the person: "Everything from your old memory now lives in the vault, and the untouched originals are archived inside it too. Want me to replace the contents of MEMORY.md with a short redirect, so there's exactly one memory going forward?" Only on a clear yes, replace `MEMORY.md`'s contents with the redirect below. Touch nothing else in the memory folder — ever.
...
6. **Jobs** _(if built)_ — "For the work you do over and over, I have a master note per job. I read one note and I've got the whole skill plus exactly the context it needs. Every correction you give makes that job sharper."
...
1. **They want ONE more piece and nothing else.** Fastest route: say the sentence to you, right here, right now. Each repo installs from one line, for example _"clone https://github.com/jaredrhod/barehands.git, then read barehands/barehands.md and set me up."_ You do it in this session and they are done.
